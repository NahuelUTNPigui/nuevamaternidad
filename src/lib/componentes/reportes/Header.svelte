<script>
    ...
    const formatOpcion = (valor,p_opciones)=>valor.length==0?"":getNombre(valor,p_opciones)
    ...
    function prepararData(item) {
        return {
            ...
            "CORTICOIDE PRENATAL": formatOpcion(item.corticoideprenatal,opciones.SINO) ,
            ...
            "DIABETES PREVIA": formatOpcion(item.diabetesprevia,opciones.SINO) || "",
            "DIABETES GESTACIONAL": formatOpcion(item.diabetesgestacional,opciones.SINO) || "",
            CORIOAMNINITIS: formatOpcion(item.crioaminitis,opciones.SINO) || "",
            "INFECCIÓN CONGÉNITA": formatOpcion(item.infeccioncongenita,opciones.SINO) || "",
            ITU: formatOpcion(item.itu,opciones.SINO) || "",
            "DESPRENDIMIENTO PLACENTA": formatOpcion(item.desprendimientoplacenta,opciones.SINO) || "",
            "SUFRIMIENTO FETAL": formatOpcion(item.sufrimientofetal,opciones.SINO) || "",
            ...
            "TIPO PARTO": formatOpcion(item.tipo_parto,opciones.TIPOPARTO) || "",
            "PARTO DOMICILIARIO": formatOpcion(item.parto_domiciliario,opciones.SINO) || "",
            "LÍQUIDO MECONIAL": formatOpcion(item.liquido_meconial,opciones.SINO) || "",
            "APGAR 1'": item.apgar_1 || "",
            "APGAR 5'": item.apgar_5 || "",
            "APGAR 10'": item.apgar_10 || "",
            REANIMACIÓN: formatOpcion(item.reanimacion,opciones.SINO) || "",
            FALLECE: formatOpcion(item.fallece,opciones.SINO) ,
            RCIU: formatOpcion(item.rciu,opciones.SINO) || "",
            ÁREA: item.expand?.area?.nombre || "",
            UNIDAD: item.expand?.unidad?.nombre || "",
            DIAGNÓSTICO: formatOpcion(item.diagnostico) || "",
            "ALIMENTACIÓN ENTERAL TRÓFICA":
                formatOpcion(item.alimentacionenteraltrofica) || "",
            ...
            "TIPO ALIMENTACIÓN ENTERAL": formatOpcion(item.tipoalimentacionenteral,opciones.TIPO_ALIMENTACION_ENTERAL) || "",
            ...
            "SEPSIS TEMPRANA": formatOpcion(item.sepsistemprana,opciones.SINO) || "",
            "GERMEN SEPSIS TEMPRANA": item.sepsistempranagermen || "",
            "ATB DÍAS SEPSIS TEMPRANA": item.sepsistempranaatbdias || "",
            "SEPSIS TARDÍA": formatOpcion(item.sepsistardia,opciones.SINO) || "",
            ...
            "TAQUIPNEA TRANSITORIA": formatOpcion(item.taquipneatransitoria,opciones.SINO) || "",
            "HIPERT. PULMONAR": formatOpcion(item.hipertpulmonar,opciones.SINO) || "",
            "ENFERMEDAD INTERSTICIAL": formatOpcion(item.enfermedadintersticial,opciones.SINO) || "",
            ...
            "CORTICOIDE INHALADO": formatOpcion(item.corticoideinhalado,opciones.SINO) || "",
            "CORTICOIDE POSTNATAL": formatOpcion(item.corticoidepostnatal,opciones.SINO) || "",
            "ÓXIDO NÍTRICO": formatOpcion(item.oxidonitrico,opciones.SINO) || "",
            DUCTUS: formatOpcion(item.ductus) || "",
            "DUCTUS DIAGNÓSTICO": formatOpcion(item.ductusdiagnostico,opciones.DUCTUS_DIAGNOSTICO) || "",
            ...
            "DUCTUS TT° QUIRÚRGICO": formatOpcion(item.ductusttoquirurgico,opciones.SINO) || "",
            "CARDIOPATÍA CONGÉNITA": formatOpcion(item.cardiopatiacongenita,opciones.SINO) || "",
            "DIAGNÓSTICO CC": formatOpcion(item.diagcc,opciones.DIAGCC) || "",
            "DOPAMINA": formatOpcion(item.inotropicosdopamina,opciones.SINO) || "",
            "DOBUTAMINA": formatOpcion(item.inotropicosdobutamina,opciones.SINO) || "",
            "ADRENALINA": formatOpcion(item.inotropicosadrenalina,opciones.SINO) || "",
            "MILRINONA": formatOpcion(item.inotropicosmilrinona,opciones.SINO) || "",
            "VASOPRESINA": formatOpcion(item.inotropicosvasopresina,opciones.SINO) || "",
            ...
            EXANGUINEOTRANSFUSIÓN: (item.exanguineotransfusio,opciones.SINOn) || "",
            ...
            "HIPOTERMIA TIPO": formatOpcion(item.hipotermiatipo,opciones.HIPOTERMIA) || "",
            ...
            "HDC DIAGNÓSTICO": formatOpcion(item.hdcdiagnostico,opciones.HDC_DIAGNOSTICO) || "",
            "HDC TT° QUIRÚRGICO": formatOpcion(item.hdcttoquirurgico,opciones.HDC_TTO_QUIRURGICO) || "",
            "DRENAJE PLEURAL": formatOpcion(item.drenajepleural,opciones.SINO) || "",
            "DRENAJE VENTRICULAR": formatOpcion(item.drenajeventricular,opciones.SINO) || "",
            ...
            "GENÉTICA VACTERL": formatOpcion(item.geneticavacterl,opciones.SINO) || "",
            "GENÉTICA TURNER": formatOpcion(item.geneticaturner,opciones.SINO) || "",
...
<header class="mb-8">
    ...
    <div
        class="hidden flex flex-col md:flex-row md:items-center justify-between gap-4"
    >
        <InputSelect
